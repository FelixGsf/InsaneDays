...
function onCreate()
	...
	makeLuaSprite('phillyGlowGradient', 'philly/gradient', -400, 350)
	...
	if curStage == 'PicoStage' then
		setProperty('phillyGlowGradient.y', getProperty('phillyGlowGradient.y') - 60)
	--	setProperty('win0.visible',true)
	end
	...
	if tag == 'Philly Glow Recreation' then
		...
		if lightId == 1 then
			...
			if active == false then
				...
				if getPropertyFromClass('PlayState', 'curStage') == 'NightR' or windowBGR then
					setProperty('win0.visible', true)
				end
