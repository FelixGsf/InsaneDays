...
class MainMenuStatesexo extends MusicBeatState
{
	public static var insaneVersion:String = 'build 3'; //This is also used for Discord RPC
	public static var curSelected:Int = 0;
public var storyShit = curSelected;
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menumod/GalleryMenu'));
	...
	//	personajes.flipX = true;
		personajes.antialiasing = ClientPrefs.globalAntialiasing;
		add(personajes);


		var splash:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image('menumod/Base'));
		...
		for (i in 0...optionShit.length)
			{
				...
				var menuItem:FlxSprite = new FlxSprite(530, (i * 190));
				...
				var scr:Float = (optionShit.length + 1) * 0.135;
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 44, 0, "Mod Base Version: " + insaneVersion, 12);
	...
	function changeItem(huh:Int = 0)
		{
			...
			for (i in 0...menuItems.members.length) // Utiliza "members" en lugar de "length"
			{
				var spr:FlxSprite = menuItems.members[i]; // Accede al elemento usando "members"
				...
				if (spr.ID == curSelected)
				{
					...
					var add:Float = 0;
					if (menuItems.members.length > 4)
					{
						// Ajusta el desplazamiento vertical según tus necesidades
						add = (menuItems.members.length - i - 1) * 8;
					}
